...
function obtain_transport_price($total_weight, $total_price, $idtransport)
{
	...
	else
	{
		...
		else
		{
			...
			$query=webtsys_query('select min_price, price from price_transport_price order by min_price ASC limit 1');
			...
			$query=webtsys_query('select price from price_transport_price where min_price>='.$min_price_last.' and idtransport='.$idtransport.' order by min_price ASC limit 1');
