...
while($arr_cat=webtsys_fetch_array($query))
{
	...
	$title_cat=$model['cat_product']->components['title']->show_formatted($arr_cat['title']);
	$description_cat=$model['cat_product']->components['description']->show_formatted($arr_cat['description']);
	$idcat=$arr_cat['IdCat_product'];
	...
	//ViewCategoriesView($title_cat, $description_cat, $idcat)
	echo load_view(array($title_cat, $description_cat, $idcat), 'shop/viewcategories');
