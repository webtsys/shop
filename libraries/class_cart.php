...
class CartClass {
	...
	public function show_cart()
	{
		...
		{
			echo '<p>'.i18n_lang('shop', 'explain_cart_options', 'Desde aquí usted puede cambiar las opciones de sus productos o eliminar su compra.').'</p>';
		}
	...
	public function add_to_cart($idproduct, $arr_details=array(), $price=0, $special_offer=0, $redirect=1)
	{
		...
		//Add product to cart, if the product have attributes, config here. Attributes are plugin. 
			...
			if($redirect==1)
			{
				...
				die( redirect_webtsys( $redirect_url, i18n_lang('common', 'redirect', 'Redirect'), i18n_lang('common', 'success', 'Success'), i18n_lang('common', 'press_here_redirecting', 'Press here for redirecting') , $arr_block) );
	...
	public function payment_gateway($iduser_shop, $idpayment)
	{
		...
		if($arr_payment['IdPayment_form']>0)
		{
			...
			if(!include(PhangoVar::$base_path.'modules/shop/payment/'.basename($arr_payment['code'])))
			{
				...
				echo i18n_lang('shop', 'error_no_proccess_payment_send_email', 'Error: no se pudo procesar el pago ni el envio del email de respuesta').': '.$config_data['portal_email'];
	...
	public function send_mail_order()
	{
		...
		if( !send_mail($arr_user['email'], i18n_lang('shop', 'your_orders', 'Su pedido'), $content_mail_user, 'html') || !send_mail(PhangoVar::$portal_email, i18n_lang('shop', 'orders', 'Pedidos'), $content_mail_admin, 'html') )
		{

			echo '<p>'.i18n_lang('shop', 'error_cannot_send_email', 'Error: no puedo enviar el email').', '.i18n_lang('shop', 'use_this_id_for_contact_with_us', 'Este es su número de pedido, especifíquelo en el email que nos envíe.').': <strong>'.$arr_order_shop['IdOrder_shop'].'</strong></p>';
