...
class CustomProductClass {
	...
	public function admin_show_options($arr_row)
	{
		...
		return '<a href="'.set_plugin_link_product($arr_row['IdProduct'], 'custom', 0).'">'.i18n_lang('shop', 'add_custom_characteristics', 'add_custom_characteristics').'</a>';
	...
	public function admin_plugin()
	{
		...
		$arr_menu[0]=array(i18n_lang('shop', 'plugin_product_admin_home', 'plugin_product_admin_home'), set_admin_link('shop', array('op' => 23, 'plugin' => $_GET['plugin'], 'element_choice' => 'product', 'op_plugin' => 0)) );
		
		$arr_menu[1]=array(i18n_lang('shop', 'add_characteristic_to_cat', 'add_characteristic_to_cat'), set_admin_link('shop', array('op' => 23, 'plugin' => $_GET['plugin'], 'element_choice' => 'product', 'op_plugin' => 1, 'id' => $_GET['id'])) );
		
		echo '<a href="'.set_admin_link('shop', array('op' => 20, 'element_choice' => 'product')).'">'.i18n_lang('shop', 'plugin_admin', 'Administración de plugins').'</a> &gt;&gt; '.menu_barr_hierarchy($arr_menu, 'op_plugin', $yes_last_link=0, $arr_final_menu=array(), $return_arr_menu=0);
		...
		switch($_GET['op_plugin'])
		{
			...
			default:
				...
				echo '<h3>'.i18n_lang('shop', 'add_product_characteristics', 'add_product_characteristics').'</h3>';
			
				PhangoVar::$model['characteristic']->create_form();
				
				PhangoVar::$model['characteristic']->forms['type']->form='SelectForm';
				
				$arr_form=array('', i18n_lang('shop', 'choose_element', 'choose_element'), '');
			...
			case 1:
				...
				if(PhangoVar::$model['characteristic']->element_exists($_GET['id']))
				{
					...
					echo '<h3>'.i18n_lang('shop', 'add_characteristic_to_cat', 'add_characteristic_to_cat').'</h3>';
			...
			case 2:
				...
				if($arr_cat['IdCharacteristic']>0)
				{
					...
					echo '<h3>'.i18n_lang('shop', 'add_product_characteristics_option', 'add_product_characteristics_option').' - '.I18nField::show_formatted($arr_cat['name']).'</h3>';
	...
	public function admin_plugin_product()
	{
		...
		if($arr_product['IdProduct']>0)
		{
			...
			$arr_menu[0]=array(i18n_lang('shop', 'add_product_characteristics', 'add_product_characteristics'), set_admin_link('shop', array('op' => 22, 'IdProduct' => $_GET['IdProduct'], 'type' => 'product', 'plugin' => $_GET['plugin'], 'op_plugin' => 0, 'idcat' => $_GET['idcat'])));
			$arr_menu[1]=array(i18n_lang('shop', 'add_new_option', 'add_new_option'), set_admin_link('shop', array('op' => 22, 'IdProduct' => $_GET['IdProduct'], 'type' => 'product', 'plugin' => $_GET['plugin'], 'op_plugin' => 1, 'idcharacteristic' => $_GET['idcharacteristic'], 'idcat' => $_GET['idcat'])) );
			
			echo '<a href="'.set_admin_link('shop', array('op' => 3, 'idcat' => $_GET['idcat'])).'">'.i18n_lang('shop', 'products', 'Productos').'</a> &gt;&gt; '.menu_barr_hierarchy($arr_menu, 'op_plugin', $yes_last_link=0, $arr_final_menu=array(), $return_arr_menu=0);
		
			echo '<h3>'.i18n_lang('shop', 'add_product_characteristics', 'add_product_characteristics').' - '.PhangoVar::$model['product']->components['title']->show_formatted($arr_product['title']).'</h3>';
			...
			switch($_GET['op_plugin'])
			{
				...
				echo up_table_config(array(i18n_lang('common', 'name', 'name'), i18n_lang('common', 'options', 'Options')));
				...
				foreach($arr_chars as $arr_char)
				{
					...
					echo middle_table_config( array(PhangoVar::$model['characteristic']->components['name']->show_formatted($arr_char['idcharacteristic']), '<a href="'.$url_set_options.'">'.i18n_lang('shop', 'edit_options', 'edit_options').'</a>') );
				...
				case 1:
					...
					echo '<p><a href="'.set_admin_link('shop', array('op' => 22, 'IdProduct' => $_GET['IdProduct'], 'type' => 'product', 'plugin' => $_GET['plugin'], 'op_plugin' => 3, 'idcharacteristic' => $_GET['idcharacteristic'], 'idcat' => $_GET['idcat'])).'">'.i18n_lang('shop', 'add_new_option', 'add_new_option').'</a></p>';
					...
					foreach($arr_chars as $id => $arr_char)
					{
						...
						{*/
							...
							echo middle_table_config( array(I18nField::show_formatted($arr_char['name']), '<a href="'.$url.'" class="check_click">'.i18n_lang('common', 'delete', 'Delete').'</a>') );
				...
				case 3:
					...
					echo '<h3>'.i18n_lang('shop', 'add_product_option', 'add_product_option').'</h3>';
...
function CustomOptionsListModel($url_options, $model_name, $id)
{
	...
	$arr_options[]='<a href="'. $url_standard.'">'.i18n_lang('shop', 'add_standard_options', 'add_standard_options').'</a>';
	$arr_options[]='<a href="'. $url.'">'.i18n_lang('shop', 'add_characteristic_to_cat', 'add_characteristic_to_cat').'</a>';
...
function SetOptionsListModel($url_options, $model_name, $id, $arr_row)
{
	...
	$arr_options[]='<a href="'. $url.'">'.i18n_lang('shop', 'delete_option_from_product', 'delete_option_from_product').'</a>';
